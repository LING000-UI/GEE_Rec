...
var endDate = "2023-09-01";

/* 定义冬季开始时间和结束时间 */
var startDate_w = "2023-11-01";
var endDate_w = "2023-12-31";

/* 定义云量掩膜系数 */
var cloud_score = 50;

/* 定义植被指数阈值 */
var tree_thresholds = 0.45;
...
var forest_mask = NDVI_s.updateMask(
  NDVI_s.gte(tree_thresholds) // Only keep pixels where class equals 2
...
var mosaic = dataset.mosaic()
                    // .updateMask(forest_mask.gte(3))
                    .setDefaultProjection({crs:_EPSG, scale:scale}); 
...
// 训练一个随机的回归RF
// 训练模型
var classifier = ee.Classifier.smileRandomForest(tree_set)
...
//   image: regression,
//   description: 'Muf_TCH_GEDI_2023',
//   scale: 10,
//   crs: _EPSG, // EPSG:32735 (WGS 84 UTM Zone 35S)
//   maxPixels: 1e13,
//   region: boundary
// });
...
// Export.image.toDrive({
//   image: S2_composite.select("B4","B3","B2","B8"),
//   description: 'S2_true_2023',
//   scale: 10,
//   crs: _EPSG, // EPSG:32735 (WGS 84 UTM Zone 35S)
//   maxPixels: 1e13,
//   region: boundary
