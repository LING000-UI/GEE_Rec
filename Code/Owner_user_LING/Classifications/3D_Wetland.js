...
var tile_coverage = ee.FeatureCollection("users/lizcanosandoval/wv-regions/tile_coverage"),
    ...
    density = ee.Image("users/lizcanosandoval/wv-regions/Region_Density");
...
var textMosaic = ui.Label({
  value: '3. Generate Land Cover Mosaic and DEM:',
  style: {
    fontWeight: 'bold', 
    fontSize: '15px',
    padding: '0px', 
...
var textPixel = ui.Label({
    value: 'Select either one or all classes to create the mosaic. Then hit the "Generate" button and wait a few seconds'+
      ' for the mosaic to load. The DEM associated to the selected area will show up in a new layer.'+
      ' Click on "Clear Selection" to clear outputs and select a new area of interest.',
    style: {
    fontSize: '13px',
    padding: '0px', 
    stretch: 'horizontal', 
...
function doMosaic() {	
  ...
  var clipDEM = filterDEM.mosaic().clip(getSelectedFeatures()).rename('dem');
  ...
  var minMax = ee.Dictionary({
    per1: rangeDem.getNumber('dem_p1'),
    per99: rangeDem.getNumber('dem_p99'),
    per50: rangeDem.getNumber('dem_p50')
  });
  ...
  //Settings for downloading mosaics
  var geometry = ee.Geometry(getSelectedFeatures().geometry(10)).bounds();

  var url = mosaicImage.getThumbURL({
    'dimensions': 1000,
    'min': 0,
    'max': 11,
    'palette': palette,
  ...
  var urlR = mosaicImageR.getThumbURL({
    'dimensions': 1000,
    'min': 0,
    'max': 11,
    'palette': palette,
  ...
  minMax.evaluate(function(dict) {
    var urlDEM = clipDEM.getThumbURL({
      'dimensions': 1000,
      'min': dict.per1,
      'max': dict.per99,
      'palette': dem_palette,
  ...
  downloadTextDEM.setUrl(urlDEM);
  downloadTextDEM.style().set({shown: true});
  });
  downloadText.setUrl(url);
  downloadTextR.setUrl(urlR);
  ...
  downloadText.style().set({shown: true});
  downloadTextR.style().set({shown: true});
...
function clearResults() {
  ...
  var reset_subPanel = ui.Panel();
  subPaneldem.widgets().reset([reset_subPanel]);
  ...
  var reset_subPanel2a = ui.Panel();
  subPanelChart.widgets().reset([reset_subPanel2a]);
  ...
  downloadText.style().set({shown: false});
  downloadTextR.style().set({shown: false});
  downloadTextDEM.style().set({shown: false});
  //Hide class palette
...
var textChart = ui.Label({
    value: 'Click on the respective button to draw a transect on the map and wait for profile chart to render.'+
...
///// DOWNLOAD MOSAIC SECTION /////

var textDownload = ui.Label({
  value: '5. Download Land Cover Mosaics and DEM (.png):',
  style: {
    fontWeight: 'bold', 
    fontSize: '15px',
    padding: '0px', 
...
var textDownloadInfo = ui.Label({
    value: 'Download mosaics in png format. Max resolution is 1000x1000 pixels. The image may take some seconds/minutes'+
      ' to show up and download. Warning: There is risk of exceeding the Earth Engine capacity if the mosaic is too large, so'+
      ' it is recommended to work with one (or a few) counties at a time if you want to download images.',
    style: {
    fontSize: '13px',
    padding: '0px', 
    stretch: 'horizontal', 
...
var downloadText = ui.Label({
  value: 'Left Map', style: {padding:'0px 0px 0px 20px'}
});
var downloadTextR = ui.Label({
  value: 'Right Map', style: {padding:'0px 0px 0px 20px'}
});
var downloadTextDEM = ui.Label({
  value: 'DEM', style: {padding:'0px 0px 0px 20px'}
});
...
var linkDesc = ui.Label({
  value: 'For Earth Engine Users:',
  style: {
    fontWeight: 'bold', 
    fontSize: '15px',
    padding: '0px', 
...
var textLinkInfo = ui.Label({
    value: 'If you have a GEE account, you can use the'+
      ' advanced interface of this App, which enables complete access to the image'+
      ' collection, download of mosaics and area calculations.',
    style: {
    fontSize: '13px',
    padding: '0px', 
    stretch: 'horizontal', 
...
Link.setUrl('https://code.earthengine.google.com/08cca563bf9fc4cce30f3f8d3cff00e2');
