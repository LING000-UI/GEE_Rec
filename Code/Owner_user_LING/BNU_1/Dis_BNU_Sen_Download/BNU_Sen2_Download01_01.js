...
// 先位01指的是进行阈值去云的,02指10%筛选
// 后位01指的是严格的10-13,02指的是时间范围的宽泛
...
for (var i=46; i<47;i++){
  ...
  var Sen_c1 = Sen.filterDate("2022-10-01","2022-10-31")
                .filterBounds(geo)
  ...
  var Sen_c4 = Sen.filterDate("2022-10-01","2022-10-31")
                .filterBounds(geo)
                .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',pixel_thresholds))
                .sort('CLOUDY_PIXEL_PERCENTAGE')
                .map(maskS2clouds)
  ...
  var img_04 = img_01.unmask(img_02).unmask(img_03).multiply(10000);
  
  
  
  Map.addLayer(img_04,{min:0,max:3000,bands:["B4","B3","B2"]},"Index"+i,1);
  ...
  Export.image.toDrive({
        ...
        description:"Sen2_SR_B4_PRJ_BB_Index_" + i + "_V01", //文件名
        scale:80, //分辨率
