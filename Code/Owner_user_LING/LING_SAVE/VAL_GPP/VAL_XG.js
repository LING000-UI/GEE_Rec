...
var table = ee.FeatureCollection("projects/ee-glj320104/assets/CITY/XiangGang");
...
// 定义感兴趣区域
var areaOfInterest = table; // 根据您的需求替换为适当的几何图形或边界

// 获取ROI名字
var Name_of_ROI = '_XiangGang_';
...
type = type.where(ELD.eq(7).and(ndvi8.gte(0.2).and(ndvi8.lt(0.5))),5);                                    // 草地
type = type.where(ELD.eq(7).and(ndvi8.gte(0.5).and(ndvi8.lte(1)).and(C2.gt(0.45))),1);                     // 落叶林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.4).and(ndvi8.lte(1)).and(C2.gte(0.35).and(C2.lte(0.45)))),2);     // 混交林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.4).and(ndvi8.lte(1)).and(C2.gte(0.05).and(C2.lt(0.35)))),3);     // 常绿林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.4).and(ndvi8.lte(1)).and(C2.lt(0.05))),4);                     // 针叶林
...
Export.image.toDrive({
  ...
  description:Name_of_ROI + 'yGPP_tif', //文件名
  scale: 10,                      //分辨率
...
Export.table.toDrive({
  ...
  description: 'dictionary_csv' + Name_of_ROI,
  fileFormat: 'CSV'
});
//-----------------------------------------------------------------------------------------------------------------------------
...
Export.table.toDrive({
  ...
  description: 'dictionary_all_csv' + Name_of_ROI,
  fileFormat: 'CSV'
});