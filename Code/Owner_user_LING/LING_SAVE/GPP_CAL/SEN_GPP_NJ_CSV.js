...
function secl(image){
    var secli = image.select('B4','B3','B2','B8','NDVI','NIRv').rename('red','green','blue','nir','NDVI','NIRv');
    return secli;
}
...
for(var q=1;q<=12;q++){
  var img = max_collection                    
  .filter(ee.Filter.eq('month',q))
  .first();
  // .select('red','blue','green');
  
  Map.addLayer(img.clip(areaOfInterest),{bands:['red','green','blue'],min:1500,max:11000},'img'+q+'true_color');
...
for(var q=1;q<=12;q++){
  ...
  Map.addLayer(img.clip(areaOfInterest),{bands:'NDVI',palette:pal2},'img'+q+'NDVI',0);
...
for(var q=1;q<=12;q++){
    ...
    Map.addLayer(img.clip(areaOfInterest),{bands:'NIRv',palette:pal2},'img'+q+'NIRv',0);
...
Map.addLayer(view_gpp,{palette:['blue','green','yellow','orange','red']},'gpp');
