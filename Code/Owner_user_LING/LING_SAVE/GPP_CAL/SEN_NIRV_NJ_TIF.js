...
// 加载12个月的最大NIRV||NDVI并下载

for(var g = 1;g <= 12;g++){
  var nirv = max_collection
  .filter(ee.Filter.eq('month',g))
  .first()
  .select('NIRv')       //NDVI
  .clip(areaOfInterest);
  
  Map.addLayer(nirv,{palette:['blue','green','yellow','orange','red']},'第'+ g +'个月的NIRv影像');
  
  Export.image.toDrive({
  image:nirv, //分类结果
  description:'nirv' + g, //文件名
  //folder: //云盘中的文件夹
  scale: 10, //分辨率
...
for(var g = 1;g <= 12;g++){
  var ndvi = max_collection
  .filter(ee.Filter.eq('month',g))
  .first()
  .select('NDVI')       //NDVI
  .clip(areaOfInterest);
  
  Map.addLayer(ndvi,{palette:['blue','green','yellow','orange','red']},'第'+ g +'个月的NDVI影像');
  
  Export.image.toDrive({
  image:ndvi, //分类结果
  description:'ndvi' + g, //文件名
  //folder: //云盘中的文件夹
  scale: 10, //分辨率
  region: areaOfInterest, 
  maxPixels:34e10 //此处值设置大一些，防止溢出
...
// Map.addLayer(mask,{palette:['red','green','cyan','yellow']},'BTO');   
...
//   description:'sum_gpp_of_pix_nj', //文件名
//   scale: 10,                      //分辨率
