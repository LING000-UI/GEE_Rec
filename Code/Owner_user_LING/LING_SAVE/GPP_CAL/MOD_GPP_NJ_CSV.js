...
function calculateNIRv(image) {
    var nirv = image.select('NDVI').multiply(image.select('sur_refl_b02')).multiply(0.0001).rename('NIRv');
    return image.addBands(nirv);
  }

// 选择波段函数（B8[nir],ndvi,nirv）
function secl(image){
    var secli = image.select('sur_refl_b02','NDVI','NIRv').rename('nir','NDVI','NIRv');
    return secli;
}
...
Export.image.toDrive({
  ...
  description:'mod_nj_yGPP_tif', //文件名
  scale: 500,                      //分辨率
