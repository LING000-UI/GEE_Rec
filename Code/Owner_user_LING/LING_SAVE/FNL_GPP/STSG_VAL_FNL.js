...
function calculateNIRv(image) {
  var nirv = image.select('NDVI').multiply(image.select('nir')).divide(10000)
  .rename('NIRv').set('system:time_start',image.get('system:time_start'));
...
function rename_NDVI(image){
  return image.select(['NDVI','d_0_NDVI','nir','d_0_nir']).rename(['NDVI_o','NDVI','nir_o','nir']);
}
function rename_NIRv(image){
  return image.select(['NIRv','d_0_NIRv']).rename(['NIRv_o','NIRv']);
}
function NIRv_mask(image){
  var mask = image.select('NIRv').gte(0);
...
var imageCollection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
  ...
  // .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',firstl));
print(imageCollection,'原始影像集');

// imageCollection = imageCollection.sort('system:time_start').map(secl).map(clipROI);
imageCollection = imageCollection.sort('system:time_start').map(secl).map(clipROI);
var allimgs=ee.ImageCollection([]);
function getmonthly (n){
  var start = ee.Date.fromYMD(year,n,1);
  var end = start.advance(1, 'month');
  var imgs=ee.ImageCollection(imageCollection.filterDate(start, end));
  var mimg = imgs.mean().set("system:time_start",start.millis());
  var b3=ee.Image().rename('red');
  var b4=ee.Image().rename('nir');
  var ming_0=ee.Image().addBands([b3,b4]).select(['red','nir']).clip(areaOfInterest);
  // return mimg}
  return ee.Algorithms.If(mimg.bandNames().size().gt(0),mimg,ming_0.set("system:time_start",start.millis()))}
for (var year = 2021; year < 2025; year++){
  var monthlyImages=ee.ImageCollection(ee.List.sequence(1,12).map(getmonthly));
  allimgs=allimgs.merge(monthlyImages);}
print("月均值",allimgs);

// var SecCollection = imageCollection.map(secl);
var ndviCollection = allimgs.map(calculateNDVI).sort('system:time_start');
print(ndviCollection,'ndviCollection')
// SG filter
var oeel=require('users/OEEL/lib:loadAll');

var Sen_filter_NDVI = oeel.ImageCollection.SavatskyGolayFilter({
  'collection':ndviCollection,
  'filter':ee.Filter.maxDifference(1000*3600*24*210,'system:time_start', null, 'system:time_start'),
  ...
  'degree':3,
  'bandOfInterest':['NDVI','nir']
});

Sen_filter_NDVI = Sen_filter_NDVI.map(rename_NDVI)
print('NDVIsg月滤波',Sen_filter_NDVI);

var nirvCollection = Sen_filter_NDVI.map(calculateNIRv).sort('system:time_start');
var Sen_filter_NIRv = oeel.ImageCollection.SavatskyGolayFilter({
  'collection':nirvCollection,
  'filter':ee.Filter.maxDifference(1000*3600*24*210,'system:time_start', null, 'system:time_start'),
  'distanceFunction':function(infromedImage,estimationImage){
          return ee.Image.constant(ee.Number(infromedImage.get('system:time_start'))
                        .subtract(ee.Number(estimationImage.get('system:time_start'))));},
...
// print('Smoothed data',ui.Chart.image.series(Sen_filter_NDVI.select(['NDVI','d_0_NDVI']),areaOfInterest, ee.Reducer.mean(),10,'system:time_start')
//   .setOptions({
...
Sen_filter_NIRv = Sen_filter_NIRv.map(rename_NIRv);
Sen_filter_NIRv = Sen_filter_NIRv.map(NIRv_mask);
print('月均值滤波',Sen_filter_NIRv);
// Map.addLayer(Sen_filter.mean(),{},'firstNDVI')

var NIRv_max = Sen_filter_NIRv.first().reduceRegion({
    reducer: ee.Reducer.minMax(),
    geometry: areaOfInterest,
    scale: 10,             // 这里填写影像分辨率
    maxPixels:1e13,        // 此处值设置大一些，防止reducer没法计算所有像元
  })
print('NIRv_max',NIRv_max);

// ///
// //SG Export
// ///
// for (var year2 = 2022;year2<=2022;year2++){
//   var VIname = 'NIRv';
//   var start_date = ee.Date('2022-01-01');
//   var end_date = ee.Date('2023-01-01');
//   for (var imonth = 1;imonth<=12;imonth = imonth+1){
//     var prestr = '';
//     if (imonth<10){
//       prestr = '0';
//     }
//     var cname2 = 'nanjing'
//     var outputname = cname2 + '_' + VIname + year2 + '_' + prestr +imonth;
//     var start = ee.Date.fromYMD(year2,imonth,1);
//     var end = start.advance(1,'month');
//     var image2 = Sen_filter_NIRv.filterDate(start,end).select('NIRv').reduce(ee.Reducer.mean()).clip(areaOfInterest);
//     Export.image.toDrive({
//         image: ee.Image(image2).select('NIRv_mean'),
//         description: outputname,
//         folder: 'XiaoLi',
//         region: areaOfInterest,
//         scale: 10,
//         maxPixels: 18000000000
//     })
//   }
// }
// 定义一个函数来添加 'year' 和 'month' 属性
function addYearAndMonth(image) {
  var date = ee.Date(image.get('system:time_start'));  // 获取时间
  var year = date.get('year');  // 提取年份
  var month = date.get('month');  // 提取月份
  
  // 将年份和月份添加为影像属性
  var imageWithAttributes = image.set({
    'year': year,
    'month': month
  });
...
// 应用该函数到整个影像集
var ndvi_collection = Sen_filter_NDVI.map(addYearAndMonth);
var mix_collection2 = Sen_filter_NIRv.map(addYearAndMonth);
allimgs = allimgs.map(addYearAndMonth)
// 打印结果查看添加的属性
...
//---------------------------------------------------addbands
// 假设你有两个影像集，分别是 collection1 和 collection2
// 定义一个函数来处理影像
function processImages1(image1) {
  // 获取 image1 的 'year' 和 'month' 属性
  var year = image1.get('year');
  var month = image1.get('month');
  
  // 从 collection2 中筛选出相同的 'year' 和 'month' 的影像
  var matchingImages = allimgs.filter(ee.Filter.eq('year', year))
                                  .filter(ee.Filter.eq('month', month));

  // 只取 collection2 中第一个匹配的影像
  var image2 = matchingImages.first();
  
  // 检查是否找到了匹配的影像
  image2 = ee.Image(image2);  // 确保 image2 是 Image 类型
  
  // 假设我们要将 image2 的所有波段添加到 image1 中
  var imageWithCopiedBands = image1.addBands(image2.select(['red','green','blue','QA60']));  // 复制所有波段

  // 返回处理后的影像
  return imageWithCopiedBands;
}

function processImages2(image1) {
  // 获取 image1 的 'year' 和 'month' 属性
  var year = image1.get('year');
  var month = image1.get('month');
  
  // 从 collection2 中筛选出相同的 'year' 和 'month' 的影像
  var matchingImages = ndvi_collection.filter(ee.Filter.eq('year', year))
                                  .filter(ee.Filter.eq('month', month));
  ...
  // 假设我们要将 image2 的所有波段添加到 image1 中
  var imageWithCopiedBands = image1.addBands(image2.select(['nir','NDVI']));  // 复制所有波段
...
// 对 collection1 中的每个影像应用该操作
var mix_collection2 = mix_collection2.map(processImages1);
mix_collection2 = mix_collection2.map(processImages2)

// 打印结果查看处理后的影像集
print(mix_collection2,'处理后的影像集');



//Delete some bands from optical features set, because they are useless in this program
function delNDVIo(image){
  var filter_OtherIndex = ee.Filter.and( 
  //创建一个过滤器用于过滤所有不需要的波段，下例中有3个波段
  ee.Filter.stringEndsWith('item', 'NDVI_o').not(),
  ee.Filter.stringEndsWith('item', 'nir_o').not())
  return image.select(image.bandNames().filter(filter_OtherIndex));
}
// 使用 map 函数将该操作应用到整个影像集
ndvi_collection = ndvi_collection.map(delNDVIo);

function delNIRvo(image){
  var filter_OtherIndex = ee.Filter.and( 
  //创建一个过滤器用于过滤所有不需要的波段，下例中有3个波段
  ee.Filter.stringEndsWith('item', 'NIRv_o').not())
  return image.select(image.bandNames().filter(filter_OtherIndex));
}
mix_collection2 = mix_collection2.map(delNIRvo);



// 打印结果查看处理后的影像集
print(ndvi_collection,'处理后的影像集ndvi_collection');
print(mix_collection2,'处理后的影像集mix_collection2');



// 筛选出年份为 2022的影像
mix_collection2 = mix_collection2
  .filter(ee.Filter.eq('year', 2022));
  
// 打印结果查看筛选后的影像集
print(mix_collection2,'2022_mix_collection2');
...
var ndvi8 = ndvi_collection
  
  .filter(ee.Filter.eq('year', 2022))
  .filter(ee.Filter.eq('month',8))
  .first().select('NDVI')
var ndvi12 = ndvi_collection
...
var sum_gpp_of_pix_cont = sum_gpp_of_pix.reduceRegion({
    reducer: ee.Reducer.minMax(),
    geometry: areaOfInterest,
    scale: 10,             // 这里填写影像分辨率
    maxPixels:1e13,        // 此处值设置大一些，防止reducer没法计算所有像元
  })
print('sum_gpp_of_pix_cont',sum_gpp_of_pix_cont);
