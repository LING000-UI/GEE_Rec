...
function secl(image){
    var secli = image.select('sur_refl_b01','sur_refl_b04','sur_refl_b03','sur_refl_b02','QA').rename('red','green','blue','nir','QA');
    return secli;
}
...
for(var i=2021;i<=2022;i++){
  ...
  for(var j=1;j<=12;j++){
    ...
    data_collection = data_collection
                                  //  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',secondl))
...
var stdpix = std_img.select('sur_refl_b01').reduceRegion({
...
var nj_5 = mix_collection2.filter(ee.Filter.eq('month',5))
                          .first();

Export.image.toDrive({
  image:nj_5.select('NDVI'), //分类结果
...
type = type.where(ELD.eq(7).and(ndvi8.lt(0.4)),9);                                                         // 裸地
type = type.where(ELD.eq(7).and(ndvi8.gte(0.4).and(ndvi8.lt(0.65))),5);                                    // 草地
type = type.where(ELD.eq(7).and(ndvi8.gte(0.65).and(ndvi8.lte(1)).and(C2.gt(0.5))),1);                     // 落叶林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.65).and(ndvi8.lte(1)).and(C2.gte(0.35).and(C2.lte(0.5)))),2);     // 混交林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.65).and(ndvi8.lte(1)).and(C2.gte(0.05).and(C2.lt(0.35)))),3);     // 常绿林
type = type.where(ELD.eq(7).and(ndvi8.gte(0.65).and(ndvi8.lte(1)).and(C2.lt(0.05))),4);                     // 针叶林
...
var year_gpp_Tgc_cal = ee.Number(year_gpp_Tgc).multiply(25).divide(100000000);     // (TgC/y)
...
function sumcalculate(image) {
  ...
  var month_gpp_cal = month_gpp.multiply(75).divide(10000000);
  //转换为TgC单位
...
function congf_urb_cal(img){
  ...
  var urb_cal = urb.multiply(75).divide(10000000);
...
function congf_cro_cal(img){
  ...
  var cro_cal = cro.multiply(75).divide(10000000);
...
function congf_tre_cal(img){
  ...
  var tre_cal = tre.multiply(75).divide(10000000);
...
function congf_gra_cal(img){
  ...
  var gra_cal = gra.multiply(75).divide(10000000);
