...
var ROI = ee.FeatureCollection("projects/ee-glj320104/assets/GPP_ROI/xn"),
    WATER = ee.FeatureCollection("projects/ee-glj320104/assets/GPP_ROI/xnwater");
...
// 制作掩膜
var mask = BTO.updateMask(BTO);


// 展示城区，红色：城区，绿色：林地,青色：草地,黄色：农作物
Map.addLayer(mask,{palette:['red','green','cayn','yellow']},'BTO');  
...
Map.addLayer(type,{palette:pal},'type');
...
function gppcalculate(image) {
  ...
  gpp = gpp.where(VEC.eq(1).or(VEC.eq(10)).or(VEC.eq(11)).or(VEC.eq(12)),0);
  gpp = gpp.where(VEC.eq(2),NIR.multiply(64.07).subtract(2.20));
  gpp = gpp.where(VEC.eq(3),NIR.multiply(59.49).subtract(2.93));
  gpp = gpp.where(VEC.eq(4),NIR.multiply(44.50).add(2.60));
  gpp = gpp.where(VEC.eq(5),NIR.multiply(64.51).subtract(1.41));
  gpp = gpp.where(VEC.eq(6),NIR.multiply(68.13).subtract(1.62));
  gpp = gpp.where(VEC.eq(7),NIR.multiply(57.66).subtract(1.31));
  gpp = gpp.where(VEC.eq(8),NIR.multiply(55.38).subtract(1.97));     // 碳3
  gpp = gpp.where(VEC.eq(9),NIR.multiply(36.18).subtract(0.87));
...
Map.addLayer(sum_gpp_of_pix,{palette:pal2},'sum_gpp_of_pix');
...
function congf_urb_cal(img){
  ...
  var Urban_Mask = LDC.neq(7);
  var Urban_area = GPP.updateMask(Urban_Mask.not()).add(0);
...
function congf_cro_cal(img){
  ...
  var Crop_Mask = LDC.neq(5);
  var Crop_area = GPP.updateMask(Crop_Mask.not()).add(0);
...
function congf_tre_cal(img){
  ...
  var Tree_Mask = LDC.neq(2);
  var Tree_area = GPP.updateMask(Tree_Mask.not()).add(0);
...
function congf_gra_cal(img){
  ...
  var Grass_Mask = LDC.neq(3);
  var Grass_area = GPP.updateMask(Grass_Mask.not()).add(0);
...
function nirv_cor_cal(img){
  ...
  var nirv_max = nirv.reduceRegion({
  ...
  var nirv_mean = nirv.reduceRegion({
  ...
  var nirv_min = nirv.reduceRegion({
...
Export.table.toDrive({
  ...
  description: 'nirv_cor_collection_NJ',
  fileFormat: 'CSV'
});